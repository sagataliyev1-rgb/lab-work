1
Ниже представлено полное выполнение заданий лабораторной работы № 2 по дисциплине «Парадигмы программирования» (язык Python). Все основные задания разобраны с кодом, таблицами состояний и комментариями.
...
| 88 | 91 | False | 91 |
| 54 | 91 | False | 91 |
6. Обязательное сравнительное задание
 * В каком варианте явно изменяется состояние total?
   В императивном варианте через явную операцию присваивания (total = total + number или total += number). В декларативном варианте функция sum() скрывает логику изменения состояния внутри себя.
 * В каком варианте программист подробнее описывает, как получить результат?
   В императивном варианте — программист пошагово задает инициализацию переменных, обход элементов цикла, проверку условий и обновление аккумулятора.
 * Какой вариант лучше демонстрирует императивную парадигму?
   Императивный вариант, так как он строится на концепции изменяемого состояния и последовательности команд.
 * Как изменяется total после обработки каждого положительного числа?
   Оно увеличивается на значение текущего положительного элемента.
 * В чем различие способа мышления в двух вариантах?
   Императивный подход отвечает на вопрос «Как сделать?» (пошаговый алгоритм управления памятью и потоком выполнения), а декларативный — «Что получить?» (описание желаемого результата).
Подскажите, пожалуйста, какой именно вариант из индивидуальных заданий (разделов 1–20) вам назначен для выполнения?
